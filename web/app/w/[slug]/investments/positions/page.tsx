...
function FilterToggle({
  ...
  return (
    <div className="flex rounded-[8px] border border-border bg-surface p-0.5">
      {options.map((o) => (
        <button
          key={o.id}
      ...
      <tbody>
        {positions.map((p) => (
          ...
          >
            ...
            <td className="px-2 py-2 text-right text-fg-muted text-[12px]">
              {formatDate(p.lastTradeDate)}
            </td>
            <td className="px-4 py-2 text-right">
              <Link
                href={`/w/${slug}/investments/instruments/${encodeURIComponent(p.symbol)}` as Route}
