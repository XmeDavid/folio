...
- No row-level security (workspace isolation at the service layer in v1).
- No partitioning strategy (defer until a workspace has enough rows to need it).
...
unique (workspace_id, id)          -- target for composite FKs (see 3.4)
```
...
Every financial row carries `workspace_id`. Isolation is enforced at two layers:

1. **Composite foreign keys.** Every workspace-scoped table has `UNIQUE (workspace_id, id)` alongside its primary key. **Every** FK from a workspace-scoped child to a workspace-scoped parent is composite — cross-table, self-referential, and user references alike:
   ...
   -- user references on workspace-scoped tables (audit_events.actor_user_id,
   ...
   This locks `workspace_id` to match on both sides at the DB layer — no service bug can create a row in workspace A that points at workspace B's data. Nullable composite FKs use MATCH SIMPLE semantics (default): the check is skipped when any referencing column is NULL, which is exactly what we want for nullable actor/parent columns.

   The only exceptions are the polymorphic link tables (`attachment_links`, `audit_events`, `source_refs`) — there is no fixed parent table for a composite FK to target. They still carry `workspace_id`; isolation is enforced by service-layer validation on write.

2. **Service layer.** Every query filters by the caller's `workspace_id`. No row-level security in v1 — RLS adds connection-pool and role-management overhead that is not justified while every workspace has a single user. Composite FKs give us the structural guarantee; service filters give us the query-shape guarantee.

Unique constraints that could collide across workspaces include `workspace_id` in the key (e.g. `(workspace_id, name)` on categories, `(workspace_id, email)` on users if we ever add multi-user-per-workspace).

### 3.5 Identity: workspaces and users

Workspaces and users are separate concepts even though v1 enforces 1:1:
- `workspaces` own financial data.
- `users` authenticate into a workspace.
- `users.workspace_id` has a `UNIQUE` constraint, enforcing one user per workspace in v1. Future household / admin / read-only-advisor roles drop the unique with a single ALTER instead of rewriting every query.
- Audit rows record `actor_user_id`; it is distinct from the row's `workspace_id`.
...
- Foreign keys named `<referenced_singular>_id` (`account_id`, `workspace_id`).
...
- `001_identity` — workspaces, users, sessions, credentials. No deps.
- `002_accounts` — → workspaces.
- `003_classification` — → workspaces, users. (No dependency on accounts.)
...
- **`workspaces`**: id, name, base_currency varchar(10), cycle_anchor_day smallint (1–31), locale text, timezone text, created_at, updated_at.
- **`users`**: id, workspace_id (**UNIQUE**), email citext unique, password_hash text nullable, display_name, last_login_at timestamptz nullable, created_at, updated_at.
...
- **`accounts`**: id, workspace_id, name, nickname, kind, currency, institution, open_date, close_date nullable, opening_balance numeric(28,8) not null default 0, opening_balance_date date, include_in_networth bool default true, include_in_savings_rate bool not null, archived_at, created_at, updated_at.
- **`account_balance_snapshots`**: id, workspace_id, account_id FK cascade, as_of timestamptz, balance, currency, source enum, note. Unique (account_id, as_of, source).
- **`reconciliation_checkpoints`**: id, workspace_id, account_id, statement_date, asserted_balance, asserted_currency varchar(10), status, drift_amount numeric(28,8) nullable, drift_currency varchar(10) nullable, resolved_at timestamptz nullable, notes. (`asserted_currency` stored with the asserted balance so the pair remains self-describing if the account's currency is ever re-interpreted.)
...
- **`categories`**: id, workspace_id, parent_id nullable self-FK on delete set null, name, color, archived_at, sort_order int, created_at, updated_at. Unique (workspace_id, parent_id, name).
- **`category_history`**: id, workspace_id, category_id FK, renamed_from text nullable, renamed_at, merged_into_category_id nullable FK, actor_user_id nullable. Composite FKs to `(workspace_id, category_id)`, `(workspace_id, merged_into_category_id)`, `(workspace_id, actor_user_id)`.
- **`merchants`**: id, workspace_id, canonical_name, logo_url text, default_category_id nullable FK, industry text, website text, notes text, archived_at, created_at, updated_at. Unique (workspace_id, canonical_name).
- **`merchant_aliases`**: id, workspace_id, merchant_id FK cascade, raw_pattern text, is_regex bool default false, created_at. Unique (workspace_id, raw_pattern).
- **`tags`**: id, workspace_id, name, color, archived_at, created_at, updated_at. Unique (workspace_id, name).
- **`categorization_rules`**: id, workspace_id, priority int, when_jsonb jsonb, then_jsonb jsonb, enabled bool default true, last_matched_at timestamptz nullable, created_at, updated_at.
...
- **`transactions`**: id, workspace_id, account_id, status, booked_at date, value_at date nullable, posted_at timestamptz nullable, amount numeric(28,8), currency, original_amount nullable, original_currency nullable, merchant_id nullable FK, category_id nullable FK (leaf only), counterparty_raw text nullable, description text nullable, notes text nullable, count_as_expense bool nullable (NULL = derive), raw jsonb, created_at, updated_at. Composite FKs to `accounts`, `categories`, `merchants` using `(workspace_id, <id>)`.
- **`transaction_lines`**: id, workspace_id, transaction_id FK cascade, amount, currency, category_id FK (leaf only), merchant_id nullable FK, note text, sort_order int. Composite FK to `(workspace_id, transaction_id)`.
- **`transaction_tags`**: (transaction_id, tag_id) composite PK, workspace_id. Composite FKs to `(workspace_id, transaction_id)` and `(workspace_id, tag_id)`.
- **`transfer_matches`**: id, workspace_id, source_transaction_id FK, destination_transaction_id nullable FK (null = outbound-to-external), fx_rate numeric(28,10) nullable, fee_amount numeric(28,8) nullable, fee_currency varchar(10) nullable, tolerance_note text, provenance, matched_at timestamptz default now(), matched_by_user_id nullable.
- **`refund_matches`**: id, workspace_id, original_transaction_id FK, refund_transaction_id FK, net_to_zero bool default true, provenance, matched_at, matched_by_user_id nullable.
- **`categorization_suggestions`**: id, workspace_id, transaction_id FK cascade, suggested_category_id FK, confidence numeric(5,4), source enum, created_at, accepted_at nullable, dismissed_at nullable.
...
Indexes: `(workspace_id, booked_at desc)`, `(account_id, booked_at desc)`, `(category_id)`, `(merchant_id)`, `(status)`.
...
- **`provider_connections`**: id, workspace_id, provider text, label, status, secrets_cipher text, metadata jsonb default '{}', consent_expires_at nullable, last_synced_at nullable, next_scheduled_sync_at nullable, last_error text, created_at, updated_at.
- **`provider_accounts`**: id, workspace_id, provider_connection_id FK, account_id nullable FK, external_account_id text, external_payload jsonb, linked_at nullable. Unique (provider_connection_id, external_account_id).
- **`import_profiles`**: id, workspace_id, name, kind, mapping jsonb, options jsonb, created_at, updated_at.
- **`import_batches`**: id, workspace_id, import_profile_id nullable FK, provider_connection_id nullable FK, source_kind, file_name text nullable, file_hash text nullable, status, summary jsonb, created_by_user_id, started_at, finished_at nullable, error text.
- **`source_refs`**: id, workspace_id, entity_type text, entity_id uuid, provider text nullable, import_batch_id nullable FK on delete set null, external_id text, raw_payload jsonb, observed_at. Unique (workspace_id, entity_type, provider, external_id) where provider is not null.

Invariants:
- Dedupe keys live in `source_refs`, not on the entity tables. A transaction can have multiple source_refs (re-import updates payload; the workspace-scoped unique key is still idempotent).
- The unique key includes `workspace_id` so two workspaces syncing the same provider + external_id never collide.
...
- **`payment_cycles`**: id, workspace_id, period_start date, period_end date, anchor, label text, status, closed_at nullable, created_at, updated_at. Unique (workspace_id, period_start).
- **`income_sources`**: id, workspace_id, name, account_id FK, amount_type, expected_amount numeric(28,8), currency, cadence jsonb, tax_hint nullable, notes text, archived_at, created_at, updated_at.
- **`recurring_templates`**: id, workspace_id, kind, name, account_id FK, dest_account_id nullable FK, category_id nullable FK, merchant_id nullable FK, amount_type, amount numeric(28,8) nullable, percentage numeric(5,2) nullable, currency, cadence jsonb, start_date, end_date nullable, share_percentage numeric(5,2) nullable, cancel_url text, notes text, archived_at, created_at, updated_at.
- **`cycle_plans`**: id, workspace_id, payment_cycle_id FK, status, summary jsonb, closed_at nullable, created_at, updated_at. Unique (payment_cycle_id).
- **`cycle_plan_lines`**: id, workspace_id, cycle_plan_id FK cascade, kind, category_id nullable FK, recurring_template_id nullable FK, income_source_id nullable FK, goal_id nullable FK, trip_id nullable FK, planned_amount numeric(28,8), currency, note, sort_order int.
- **`planned_events`**: id, workspace_id, cycle_plan_line_id nullable FK, recurring_template_id nullable FK, kind (shared enum with recurring), account_id, dest_account_id nullable, category_id nullable, merchant_id nullable, planned_for date, amount, currency, status, executed_transaction_id nullable FK, created_at, updated_at.
- **`action_items`**: id, workspace_id, planned_event_id FK cascade, instruction text, due_at date, status, done_at nullable, notes text, created_at, updated_at.
- **`rollover_policies`**: id, workspace_id, category_id FK, behavior, cap_amount numeric(28,8) nullable, cap_currency varchar(10) nullable, overspend, created_at, updated_at. Unique (workspace_id, category_id).
- **`planned_event_matches`** (relocated from 5.4 — needs both `planned_events` here and `transactions` from 004): id, workspace_id, planned_event_id FK, transaction_id FK, provenance, matched_at.
...
- **`goals`**: id, workspace_id, name, target_amount numeric(28,8), currency, deadline date nullable, priority int, status, type, parent_goal_id nullable self-FK, auto_redirect_on_reach_goal_id nullable self-FK, notes text, archived_at, created_at, updated_at.
- **`goal_accounts`**: (goal_id, account_id) composite PK, workspace_id, share_percentage numeric(5,2) nullable.
- **`goal_allocations`** (append-only): id, workspace_id, goal_id FK, account_id FK, balance numeric(28,8), currency, as_of timestamptz, source.
- **`goal_contributions`**: id, workspace_id, goal_id FK, transaction_id nullable FK, planned_event_id nullable FK, amount numeric(28,8), currency, applied_at timestamptz.
- **`savings_rules`**: id, workspace_id, name, priority int, trigger, trigger_config jsonb, action, action_config jsonb, enabled bool default true, last_fired_at nullable, created_at, updated_at.
...
- **`instruments`** (**global, no workspace_id**): id, symbol text, isin text nullable, name, asset_class, currency, exchange text nullable, metadata jsonb, active bool default true, created_at, updated_at. Unique (isin) where isin is not null; unique (symbol, exchange) where isin is null.
- **`investment_accounts`**: account_id PK references accounts(id) on delete cascade, workspace_id, cost_basis_method default `fifo`, default_tax_lot_strategy text nullable.
- **`investment_trades`**: id, workspace_id, account_id FK, instrument_id FK, side, quantity numeric(28,8), price numeric(28,8), currency, fee_amount numeric(28,8) default 0, fee_currency varchar(10), trade_date, settle_date nullable, linked_cash_transaction_id nullable FK, created_at, updated_at.
- **`investment_lots`**: id, workspace_id, account_id FK, instrument_id FK, acquired_at date, quantity_opening numeric(28,8), quantity_remaining numeric(28,8), cost_basis_per_unit numeric(28,8), currency, source_trade_id FK, closed_at nullable, created_at.
- **`investment_lot_consumptions`**: id, workspace_id, lot_id FK, sell_trade_id FK, quantity_consumed numeric(28,8), realised_gain numeric(28,8), currency, consumed_at date.
- **`dividend_events`**: id, workspace_id, account_id FK, instrument_id FK, ex_date date, pay_date date, amount_per_unit numeric(28,8), currency, total_amount numeric(28,8), tax_withheld numeric(28,8) default 0, linked_cash_transaction_id nullable FK, created_at.
- **`corporate_actions`**: id, workspace_id nullable, account_id nullable FK, instrument_id FK, kind, effective_date date, payload jsonb, applied_at timestamptz nullable, created_at.
...
- **`allocation_buckets`**: id, workspace_id, name, parent_bucket_id nullable self-FK, target_percentage numeric(5,2) nullable, created_at, updated_at.
- **`position_bucket_allocations`**: (account_id, instrument_id, bucket_id) composite PK, workspace_id, share_percentage numeric(5,2) default 100.
- **`target_allocations`**: id, workspace_id, bucket_id FK, target_percentage numeric(5,2), effective_from date, created_at.
...
- `instruments` and `instrument_prices` are global; workspace deletion does not cascade into them.
...
- **`assets`**: id, workspace_id, account_id FK **UNIQUE**, category, description text, acquired_date, acquired_cost numeric(28,8), currency, disposal_date nullable, disposal_amount nullable, disposal_currency varchar(10) nullable, valuation_method, notes text, created_at, updated_at.
- **`asset_valuations`**: id, workspace_id, asset_id FK cascade, as_of date, value numeric(28,8), currency, source, note.
- **`asset_events`**: id, workspace_id, asset_id FK cascade, kind, occurred_at date, amount numeric(28,8) nullable, currency varchar(10) nullable, linked_transaction_id nullable FK, note, created_at.
- **`asset_depreciation_schedules`**: id, workspace_id, asset_id FK cascade UNIQUE, method, start_date, end_date nullable, salvage_value numeric(28,8) nullable, rate numeric(7,4) nullable, schedule_jsonb jsonb nullable.
- **`retirement_contribution_limits`** (**reference data, no workspace_id**): id, country char(2), year int, pillar, amount numeric(28,8), currency. Unique (country, year, pillar).
- **`mortgage_schedules`**: id, workspace_id, account_id FK **UNIQUE** (account of kind `mortgage`), original_principal numeric(28,8), currency, interest_rate numeric(7,4), term_months int, start_date, created_at, updated_at.
- **`mortgage_payments`**: id, workspace_id, mortgage_schedule_id FK cascade, due_on date, principal_amount numeric(28,8), interest_amount numeric(28,8), currency, linked_transaction_id nullable FK, status, paid_at timestamptz nullable, created_at, updated_at.
...
- **`people`**: id, workspace_id, name, email citext nullable, phone text nullable, notes text, archived_at, created_at, updated_at.
- **`trips`**: id, workspace_id, name, destinations text[], start_date, end_date, overall_budget numeric(28,8) nullable, currency, status, notes text, created_at, updated_at.
- **`trip_budgets`**: id, workspace_id, trip_id FK cascade, category, custom_label text nullable, budget_amount numeric(28,8), currency. Unique (trip_id, category, custom_label).
- **`trip_participants`**: id, workspace_id, trip_id FK cascade, person_id nullable FK, is_self bool default false, display_name, share_default, created_at.
- **`trip_transaction_links`**: (trip_id, transaction_id) composite PK, workspace_id, trip_category, created_at.
- **`split_bill_events`**: id, workspace_id, transaction_id nullable FK, trip_id nullable FK, total_amount numeric(28,8), currency, allocation_method, note text, state enum, created_at, settled_at nullable.
- **`split_bill_allocations`**: id, workspace_id, split_bill_event_id FK cascade, participant_trip_id nullable FK (trip_participants.id), person_id nullable FK, amount_owed numeric(28,8), currency, item_description text nullable, transaction_line_id nullable FK.
- **`receivables`**: id, workspace_id, counterparty_person_id nullable FK, counterparty_label text, direction, amount numeric(28,8), currency, origin, origin_event_id uuid nullable, due_date date nullable, status, notes text, created_at, updated_at, settled_at nullable.
- **`settlements`**: id, workspace_id, receivable_id FK cascade, settling_transaction_id nullable FK, amount numeric(28,8), currency, settled_at timestamptz default now(), note.
- **`reimbursement_claims`**: id, workspace_id, transaction_id FK, employer_or_counterparty text, claim_status, submitted_at nullable, paid_at nullable, paid_transaction_id nullable FK, notes text, created_at, updated_at.
...
- **`wishlist_items`**: id, workspace_id, name, estimated_price numeric(28,8) nullable, currency, url text nullable, notes text, priority int, status, linked_goal_id nullable FK, archived_at, created_at, updated_at.
- **`wishlist_price_observations`**: id, workspace_id, wishlist_item_id FK cascade, observed_at timestamptz, price numeric(28,8), currency, source, scraper_run_id uuid nullable, note.
- **`wishlist_purchase_links`**: id, workspace_id, wishlist_item_id FK **UNIQUE**, transaction_id FK, purchased_at date, actual_amount numeric(28,8), currency, variance numeric(28,8).
...
- **`attachments`**: id, workspace_id, filename, content_type, size_bytes bigint, storage_backend, storage_key text, sha256 bytea, uploaded_by_user_id, uploaded_at timestamptz default now(), ocr_status default `none`, created_at. Unique (workspace_id, sha256) — dedupe same file.
- **`attachment_links`**: id, workspace_id, attachment_id FK cascade, entity_type text, entity_id uuid, linked_at timestamptz default now(), linked_by_user_id. Unique (attachment_id, entity_type, entity_id).
- **`ocr_documents`**: id, workspace_id, attachment_id FK cascade UNIQUE, text_content text, extracted_jsonb jsonb, processed_at, engine text, confidence numeric(5,4).
- **`saved_searches`**: id, workspace_id, user_id FK, name, filter_jsonb jsonb, pinned bool default false, created_at, updated_at.
- **`audit_events`**: id, workspace_id, entity_type text, entity_id uuid, action, actor_user_id nullable FK, before_jsonb jsonb nullable, after_jsonb jsonb nullable, reason text nullable, ip inet nullable, user_agent text nullable, occurred_at timestamptz default now().

Invariants:
- Attachments are physically stored once per `(workspace_id, sha256)` tuple; `attachment_links` is the m-to-n to entities.
...
Indexes: `(workspace_id, entity_type, entity_id, occurred_at desc)` on `audit_events`.
...
- **`fx_rates`** (**global, no workspace_id**): id, base_currency varchar(10), quote_currency varchar(10), as_of date, rate numeric(28,10), source, fetched_at timestamptz default now(). Unique (base_currency, quote_currency, as_of, source).
- **`networth_snapshots`**: id, workspace_id, as_of date, total_value numeric(28,8), base_currency varchar(10), breakdown_jsonb jsonb, computed_at timestamptz, created_at. Unique (workspace_id, as_of).
- **`event_markers`**: id, workspace_id, as_of date, kind, label text, note text, created_at.
- **`report_exports`**: id, workspace_id, requested_by_user_id FK, kind, params_jsonb, status, file_attachment_id nullable FK, requested_at, completed_at nullable, expires_at nullable, error text.
- **`export_templates`**: id, workspace_id, name, kind, definition_jsonb, created_at, updated_at.
...
- **`notification_rules`**: id, workspace_id, name, event_kind text, config_jsonb, enabled bool default true, digest_mode default `realtime`, created_at, updated_at.
- **`notification_events`**: id, workspace_id, rule_id nullable FK, event_kind text, subject_entity_type text, subject_entity_id uuid, payload_jsonb, occurred_at timestamptz default now().
- **`notification_deliveries`**: id, workspace_id, notification_event_id FK cascade, channel, destination_id uuid nullable, status default `queued`, attempted_at timestamptz nullable, delivered_at nullable, read_at nullable, error text. Unique (notification_event_id, channel, destination_id).
- **`notification_preferences`**: id, workspace_id, user_id FK, event_kind text, channel, enabled bool default true, digest_override notification_digest_mode nullable. Unique (user_id, event_kind, channel).
- **`webhook_destinations`**: id, workspace_id, name, url text, secret_cipher text, enabled bool default true, last_success_at nullable, last_error text, created_at, updated_at.
...
- Identity & workspace (workspaces vs users).
...
- Row-level security (defer until multi-user-per-workspace arrives).
- Table partitioning (defer until a workspace has enough rows).
...
- Every workspace-scoped table has `UNIQUE (workspace_id, id)`; every cross-table FK to a workspace-scoped parent is composite `(workspace_id, <ref>) REFERENCES parent(workspace_id, id)`.
