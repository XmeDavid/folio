...
- **Every amount carries a currency.** Base currency per user (default CHF) for reporting rollups; original currency always preserved.
...
- **Opening balance + start date** (mandatory). No stats or networth calculations extend before the start date. Each account can have its own start date (Revolut since 2019, PostFinance since Dec 2024).
...
### Post-v1 scope (listed here for reference, not excluded from the vision)

- Duplicate-exposure warnings (VUAA + VUSA, VT + regional overlap).
- Fractional-share / dust detection ("close this 0.08-share position").
- Watchlist with price alerts.
- Crypto wallet scraping via public addresses.

### Explicitly out of scope

- AI-generated rebalancing advice.
- Auto-generated portfolio commentary PDFs (the `portfolio_analysis.md` was a one-shot LLM artifact, not a product feature).
...
- **Localisation**: English first. Swiss locales (de-CH, fr-CH, it-CH) as follow-ons given the target market.
- **Number / date formats** — locale-aware.
...
## Open questions & decisions to make later (not features, tracked here so they don't get lost)

- **AI backend selection at v1**: Ollama self-host vs. BYO API key vs. both — depends on available model quality at implementation time.
- **External price provider** for manual equity positions: stooq vs. Alpha Vantage vs. Yahoo — ToS and reliability review needed closer to implementation.
- **External enrichment provider** for merchants (Plaid / Ntropy / etc.) vs. local rules forever — business decision.
- **Localisation rollout order** beyond English.
- **Notification delivery channels** matrix (web push, email, external) — per-channel feature prioritisation.
- **Tax-year preset** start date — user-configurable? Or fixed to calendar year (works for CH)?
