# Folio — Authentication, Users, and Multi-Workspace Design

**Status:** Approved for implementation
**Date:** 2026-04-24
**Source:** `FEATURE-BIBLE.md` (§1 Overview, §39 Security), `docs/superpowers/specs/2026-04-24-folio-domain-v2-design.md`
**Supersedes:**
- The 1:1 workspace↔user invariant on `users.workspace_id` in `backend/db/migrations/20260424000001_identity.sql`.
- The `X-Workspace-ID` / `X-User-ID` header stand-in middleware `httpx.RequireWorkspace` in `backend/internal/httpx/httpx.go`.
- The password-less `Bootstrap` handler on `backend/internal/identity/service.go`.

## 1. Goal

Land the identity, authentication, and authorisation layer for the final product — self-hosted and SaaS deployments from the same codebase. This is not an MVP slice: password + passkey + TOTP, invites, email verification, password reset, step-up MFA, soft-deleted workspaces, and per-workspace membership roles are all in scope from day one.

After this lands:
- There is no `X-Workspace-ID` dev bridge anywhere in the stack.
- A user can belong to many workspaces with a role per membership; workspaces can have many owners.
- Login uses passkeys-first or password-first at the user's choice, with optional TOTP as 2FA.
- Workspace-scoped URLs (`/t/{slug}/…`) replace the global dashboard shape.

## 2. Non-goals

- **Billing / plans / metering.** SaaS commercials come later; the workspace has no billing column yet.
...
- **Admin impersonation** — viewing a workspace's financial data as if a member. Privacy/legal-sensitive for fintech; wants its own spec (reason-required, tightly audited, read-only, feature-flagged).
- **Cross-workspace aggregate views** (e.g. "all my net worth across Personal + Household"). Different base currencies, different cycle anchors — non-trivial and out of scope.
...
- Composite FK target `UNIQUE (workspace_id, id)` on every workspace-scoped table.
...
- **Remove** `workspace_id uuid` column, its `UNIQUE (workspace_id)` constraint, and the composite `UNIQUE (workspace_id, id)` target.
- **Add:**
  - `email_verified_at timestamptz` — set by the email-verification flow.
  - `last_workspace_id uuid references workspaces(id) on delete set null` — last-used workspace for post-login redirect.
  - `is_admin boolean not null default false` — instance-level admin flag for the admin console (§11). Orthogonal to workspace membership; granted via CLI (`folio-admin grant`) or first-run env bootstrap.
...
- Soft-delete partial index: `create index workspaces_deleted_at_idx on workspaces (deleted_at) where deleted_at is not null;` — supports the hard-delete sweeper job.
- Existing `workspaces` FKs elsewhere (every financial table's `workspace_id`) continue pointing at `workspaces(id)` unchanged; they'll see soft-deleted workspaces unless the service layer filters on `deleted_at is null`, which it must do everywhere except the restore endpoint.
...
-- A user can belong to many workspaces; a workspace can have many users
...
1. **Every workspace has ≥1 owner at all times.** Demote / remove / leave is blocked if it would drop `workspace_memberships_owners` count to zero. The partial index makes the check a single-digit-ms lookup.
2. **A user cannot leave their last workspace.** UX instead guides them to "create a Personal workspace" or "delete account". Keeps the "every user has ≥1 membership" invariant trivially true (signup already establishes it).
3. **Cross-workspace data references are impossible.** Financial rows keep the composite-FK convention from the v2 design.
4. **Soft-deleted workspaces are invisible.** All service-layer reads filter `deleted_at is null` except the explicit "restore" and "list deleted" paths.

### 3.5 Permissions matrix

| Action | Owner | Member |
|---|:---:|:---:|
| Read workspace data (accounts, transactions, goals, …) | ✓ | ✓ |
...
| Leave workspace (not if last owner) | ✓ | ✓ |
...
- **API:** UUID-based — `/api/v1/t/{workspaceId}/…`. Slug is strictly a web concern; the API never dereferences a slug.
- **Translation:** `GET /api/v1/me` returns `{user, workspaces: [{id, slug, name, role}, …]}`. The web client resolves slug → UUID locally on navigation.
- **Slug generation:** auto-slugify from workspace name at create time; on collision append `-2`, `-3`, … If a user types a slug explicitly and it collides, return a validation error and let them choose.

### 4.2 Auth endpoints (public, not workspace-scoped)

| Route | Purpose |
|---|---|
| `POST /api/v1/auth/signup` | Create user + Personal workspace + owner membership + session. Accepts optional `inviteToken`; when present and email matches, also creates a membership in the invited workspace. |
| `POST /api/v1/auth/login` | Email + password. Returns `{mfa_required, challenge_id}` if MFA is enrolled; otherwise sets the session cookie and returns user+workspaces. |
...
| `GET  /api/v1/auth/invites/{token}` | Preview an invite: workspace name, inviter display name, role, expiry. No auth required. |
| `POST /api/v1/auth/invites/{token}/accept` | Authenticated — must match invite email; creates membership, consumes token. |

### 4.3 Authenticated, non-workspace-scoped endpoints

| Route | Purpose |
|---|---|
| `GET  /api/v1/me` | Current user + workspace list (id, slug, name, role). |
...
### 4.4 Workspace-scoped endpoints (under `/api/v1/t/{workspaceId}`)
...
Everything else in `/api/v1/t/{id}/…` (accounts, transactions, categories, etc.) is the existing financial surface, re-mounted under this workspace-scoped prefix instead of the current flat `/api/v1/accounts` shape.
...
2. **`RequireMembership`** — extracts `{workspaceId}` from the URL, looks up `(workspace_id, user_id)` in `workspace_memberships`, attaches the role. **Returns 404, not 403, on a miss** to avoid leaking workspace existence.
...
Public auth routes skip `RequireSession`; authenticated-non-workspace routes use `RequireSession` only.

### 4.6 Workspace scope enforcement (service layer)

Every service method that reads or writes workspace-scoped data takes `workspaceID` as its first argument (after `context.Context`). Handlers read it from the context populated by `RequireMembership`, never from the request body. Soft-deleted workspaces are filtered out of all reads except explicit restore/list-deleted paths.
...
- Soft-delete a workspace (and restore).
...
- User removed from a workspace → **no** session revocation; their other workspace memberships are untouched.
...
| Workspace invite | 7 d | Link: `folio.app/accept-invite/{token}`. |

Verify / reset / email-change tokens live in `auth_tokens`; invite tokens in `workspace_invites`. All tokens stored as SHA-256 hashes; plaintext appears only in the emailed URL.

**Email verification gating:** unverified users can access the dashboard and read their workspace, but the following actions require `email_verified_at IS NOT NULL`:
...
| All state-changing workspace routes | 60 / min / user |
...
Written to `audit_events` (existing schema) with `actor_user_id`, `workspace_id` (nullable for user-scoped events), `action`, `entity_type`, `entity_id`, `before`, `after`, `at`.
...
| workspace | `workspace.created`, `workspace.renamed`, `workspace.settings_changed`, `workspace.deleted`, `workspace.restored` |
| membership | `member.invited`, `member.invite_revoked`, `member.invite_accepted`, `member.role_changed`, `member.removed`, `member.left` |
| admin | `admin.granted`, `admin.revoked`, `admin.bootstrap_granted`, `admin.viewed_user`, `admin.viewed_workspace`, `admin.viewed_audit`, `admin.retried_job`, `admin.resent_email` |

Failed-login events are keyed by email (not user) to avoid creating a row for every typo. User-scoped events (password change, session revoke, etc.) are written with `workspace_id = NULL`. Admin events are always `workspace_id = NULL` (they're cross-workspace by definition).
...
## 10. Soft delete and the workspace-sweeper

- `DELETE /api/v1/t/{id}` sets `workspaces.deleted_at = now()`. The row survives; all dependent data is untouched.
- A periodic River job (daily) hard-deletes workspaces with `deleted_at < now() - interval '30 days'`. Hard delete cascades via existing FKs.
- `POST /api/v1/t/{id}/restore` clears `deleted_at`. Owner-only. Step-up required.
- While soft-deleted, a workspace is invisible to every API path except explicit restore / list-deleted.

## 11. Admin console

Read-only debugging surface + a small set of operational writes for the person who runs the Folio instance (self-host owner or SaaS operator). Orthogonal to workspace roles — admin status is a user-level boolean (`users.is_admin`), not a workspace role. An admin has no default read/write access to any workspace's financial data; accessing workspace data would require impersonation, which is deferred (§2 non-goals).
...
- **Last-admin guard:** both the CLI and the `/admin/users/{id}/revoke-admin` endpoint refuse if the target would become the last admin (parallel to the last-owner workspace invariant).
...
| Route | Purpose |
|---|---|
| `GET /api/v1/admin/workspaces` | Paginated list + search by name/slug/id. |
| `GET /api/v1/admin/workspaces/{id}` | Detail: member count, settings, `deleted_at`, last activity. Metadata only — no financial rows. |
...
| `GET /api/v1/admin/audit` | Cross-workspace feed of `audit_events` with filters (actor, workspace, action, time range). |
...
Reads audit too. Every admin route writes an audit event with `workspace_id = NULL` — fintech staff access to customer data (even metadata) should always be answerable: "who looked at this, and when?". Actions: see the `admin` row in §8.3.
...
- Drop `users.workspace_id`.
- Add `email_verified_at`, `last_workspace_id`, `is_admin` to `users`.
- Add `slug`, `deleted_at` to `workspaces`.
- Add `last_seen_at`, `reauth_at` to `sessions`.
- Drop `totp_credentials.recovery_codes_cipher`.
- Add `workspace_memberships`, `workspace_invites`, `auth_tokens`, `auth_recovery_codes`.
- Add the `workspace_role` enum.

All downstream migrations need a review pass to update anything that currently FK-ed `users(workspace_id, id)` — there shouldn't be any in the tree today, but a grep is required as part of implementation.

The existing `backend/internal/identity.Bootstrap` code and `backend/internal/httpx.RequireWorkspace` middleware are deleted entirely and replaced by the new `backend/internal/auth` package.
...
- Authed, non-workspace: `/settings/account`, `/settings/security`, `/settings/sessions`, `/workspaces` (workspace picker / create new).
- Workspace-scoped: `/t/{slug}/…` covers the existing dashboard, accounts, transactions, categories, etc., plus `/t/{slug}/settings/members`, `/t/{slug}/settings/invites`, `/t/{slug}/settings/workspace`.
- Admin (only visible to `is_admin=true` users): `/admin/workspaces`, `/admin/users`, `/admin/audit`, `/admin/jobs`. Linked from the user menu with a distinct "Admin" badge so admins don't forget what hat they're wearing.
- Workspace switcher lives in the top bar / sidebar; backed by `/me.workspaces`.
...
- **Billing & plans** — workspaces will grow `billing_*` columns and a separate `plans` table.
- **SSO / social login** — adds an `auth_identities(provider, provider_user_id, user_id)` table; login ceremony branches.
- **Admin impersonation** — "view this workspace as a member" for support. Feature-flagged, reason-required, fully audited, read-only. Complements the admin console in §11.
- **Cross-workspace aggregate views** — "show me net worth across all my workspaces".
